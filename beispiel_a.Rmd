...
### because we want to know the new population size of wildtype genotypes, else we would just get the initial population size of N_A (mutant genotype) again. 
* In the life cycle as defined above, do individuals reproduce first and then mutate, or vice versa?
### first they reproduce by drawing a number according to poisson distribution  (=offspring), then a mutation is drawn by poisson distribution.
...
### Although there are initially no mutants, offspring has a mutation rate, so the next generation we almost always end up with mutations. 100 individuas, all reproduce -> mut_rate = 0.01 so normally 1 mutants per generation
* What does the function `c` do?
### it stores output values in a vector: 2 values per generation, return and c puts 2 values together in a string. 
...
###pop_new: pop. size of wildtype and mutants seperate per gen. , pop_vector= stores each pop_new output 
* What is the condition that breaks the loop good for? What are its disadvantages?
### define "success" or "fail" save time; tables of different lengths!
* What does the function `tail` do?
### print out last 5 rows of data table; check code if everything behaves as expected
...
### because of loops: code does get multiple repetitions/runs, so the "path" is longer
...
### so we dont confuse values for each parameters: mistakes much less likely to happen; and for formulas or codes, it is easier if you dont have to look up which value is what parameter. AND: change value only once, play around/adjust with model much more easily
* What does the function `which` do?
### "which" function is used to return position of specified values in a vector
(as.numeric: used to turn character vector into numeric vector)
